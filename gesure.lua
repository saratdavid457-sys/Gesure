...
local SafePosition = CFrame.new(-183.67051696777344, 295.9679870605469, -998.7326049804688)
...
local Config = {
    ["TheBronx"] = {
        ...
        ["TeleportationList"] = {
            ...
            ["Capital One Bank 🏦"] = CFrame.new(-222.43960571289062, 283.8095397949219, -1202.06103515625);
            ...
            ["Basketball Court 🏀"] = CFrame.new(-941.1542358398438, 253.97064208984375, -408.6098937988281);
            ...
            ["Safe 🔒"] = CFrame.new(-183.67051696777344, 295.9679870605469, -998.7326049804688);
    ...
    LPH_JIT_MAX(function()
        ...
        if Game_Name == "South Bronx" then
            ...
            HideUI = LPH_NO_VIRTUALIZE(function(Title, Timing)
                ...
                textLabel.Text = '<font color="rgb(0,163,224)">Ges</font>ure\n' .. Title
                ...
                if Timing then
                    task.spawn(function()
                        ...
                        while tick() < endTime do
                            ...
                            textLabel.Text = string.format(
                                '<font color="rgb(0,163,224)">Ges</font>ure\n%s\nplease wait : <font color="rgb(0,163,224)">%.2f</font> seconds',
                                Title, math.max(timeLeft, 0)
                            )
...
-- Find first instance of tool
local function getSingleToolInstance(toolName)
...
local function waitToolGone(toolName, timeout)
...
local function processTool(tool)
    if not tool or Processing or not ScriptEnabled then return end
    Processing = true
    ...
    conn = ReplicatedStorage.MarketItems.ChildAdded:Connect(function(item)
        if gotMatch then return end
        if item.Name == toolName then
            local ok, owner = pcall(function() return item:WaitForChild("owner").Value end)
    ...
    pcall(function() ReplicatedStorage.ListWeaponRemote:FireServer(toolName, 99999) end)
    task.wait(0.25)

    pcall(function() ReplicatedStorage.BackpackRemote:InvokeServer("Store", toolName) end)
    task.wait(2)

    pcall(function() ReplicatedStorage.BuyItemRemote:FireServer(toolName, "Remove", ToolId) end)

    waitToolGone(toolName, 4)

    pcall(function() ReplicatedStorage.BackpackRemote:InvokeServer("Grab", toolName) end)
    conn:Disconnect()

    local newTool = getSingleToolInstance(toolName)
    if newTool and humanoid then
        pcall(function() humanoid:EquipTool(newTool) end)
    end
    ...
    while ScriptEnabled do
        if not Processing then
            ...
            elseif LastToolName then
                local t = getSingleToolInstance(LastToolName)
                if t then
...
DupingSection:toggle({
    ...
    callback = function(state)
        ...
        if state then
            SendNote("Successfully Enabled")
            task.spawn(watcher)
        else
            SendNote("Successfully Disabled!")
        end
    end
})
...
button.MouseButton1Click:Connect(function()
    if setclipboard then
        setclipboard("https://discord.gg/B6yCcS2SGP")
    elseif toclipboard then
        toclipboard("https://discord.gg/B6yCcS2SGP")
    end
    button.Text = "Copied!"
end)
